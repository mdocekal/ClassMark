...
class Validator(Plugin):
    ...
    def run(self, dataSet: DataSet, classifiers: List[Classifier], data: np.array, selectedAttributes: List[str],
            labels: np.array, extMap: List[FeatureExtractor], featSel=None,
            subStepCallback: Callable[[str], None] = None):
        """
        ...
        :type data: np.array
        :param selectedAttributes: Name of attributes from which data came from.
        :type selectedAttributes: List[str]
        ...
        :return: Generator of tuple
            Tuple[step,classifier,predictedLabels,importantFeatures,importanceOfFeatures,realLabels,
                selectedSamplesIndicesForTest, dict of times, samples stats]
        :rtype: Iterator[Tuple[int,Classifier,np.array,Optional[np.array],Optional[np.array],np.array,np.array,Dict[TimeDuration, float],Dict[SamplesStats,int]]]
        """
        ...
        for step, (trainIndices, testIndices) in enumerate(self._splitter(data, labels)):
            ...
            if subStepCallback is not None: subStepCallback(
                "{}/{}: Extracting features for train set.".format(stepToShow, numOfSteps))
            ...
            trainFeatures, trainFeaturesNames = self._featuresStep(data[trainIndices], selectedAttributes, extMap, trainLabels, fit=True)

            times[self.TimeDuration.FEATURE_EXTRACTION_TRAIN_PROC] = time.process_time() - times[
                self.TimeDuration.FEATURE_EXTRACTION_TRAIN_PROC]
            times[self.TimeDuration.FEATURE_EXTRACTION_TRAIN] = time.time() - times[
                self.TimeDuration.FEATURE_EXTRACTION_TRAIN]
            ...
            # feature selection
            if subStepCallback is not None: subStepCallback(
                "{}/{}: Selecting features for train set.".format(stepToShow, numOfSteps))
            ...
            for s in featSel:
                ...
                trainFeaturesNames = trainFeaturesNames[s.getSupport()]

            times[self.TimeDuration.FEATURE_SELECTION_TRAIN_PROC] = time.process_time() - times[
                self.TimeDuration.FEATURE_SELECTION_TRAIN_PROC]
            times[self.TimeDuration.FEATURE_SELECTION_TRAIN] = time.time() - times[
                self.TimeDuration.FEATURE_SELECTION_TRAIN]
            ...
            if subStepCallback is not None: subStepCallback(
                "{}/{}: Extracting features for test set.".format(stepToShow, numOfSteps))
            ...
            testFeatures, _ = self._featuresStep(data[testIndices], selectedAttributes, extMap)

            times[self.TimeDuration.FEATURE_EXTRACTION_TEST_PROC] = time.process_time() - times[
                self.TimeDuration.FEATURE_EXTRACTION_TEST_PROC]
            times[self.TimeDuration.FEATURE_EXTRACTION_TEST] = time.time() - times[
                self.TimeDuration.FEATURE_EXTRACTION_TEST]
            ...
            if subStepCallback is not None: subStepCallback(
                "{}/{}: Selecting features for test set.".format(stepToShow, numOfSteps))
            ...
            times[self.TimeDuration.FEATURE_SELECTION_TEST_PROC] = time.process_time() - times[
                self.TimeDuration.FEATURE_SELECTION_TEST_PROC]
            times[self.TimeDuration.FEATURE_SELECTION_TEST] = time.time() - times[
                self.TimeDuration.FEATURE_SELECTION_TEST]
            ...
            for classifier in classifiers:
                ...
                if subStepCallback is not None: subStepCallback(
                    "{}/{}: Training {}.".format(stepToShow, numOfSteps, classifier.getName()))
                ...
                if classifier.featureImportanceShouldBeShown():
                    try:
                        # let's look at the feature importance
                        Logger().log("The most important features are:")
                        Logger().log("feature\timportance")

                        for name, importance in classifier.topFeatures(trainFeaturesNames):
                            Logger().log(f"{name}\t{importance}")

                        Logger().log("---")

                    except NotImplementedError:
                        # current classifier does not provide this functionality
                        pass

                if subStepCallback is not None: subStepCallback(
                    "{}/{}: Testing {}.".format(stepToShow, numOfSteps, classifier.getName()))
                times[self.TimeDuration.TEST] = time.time()
                times[self.TimeDuration.TEST_PROC] = time.process_time()
                # predict the labels
                importantFeatures = None
                importanceOfFeatures = None

                if classifier.featureImportanceShouldBeShown():
                    predictedLabels, importantFeatures, importanceOfFeatures = \
                        classifier.classifyShowTopFeatures(testFeatures, trainFeaturesNames)
                else:
                ...
                yield (step, classifier, predictedLabels, importantFeatures, importanceOfFeatures, labels[testIndices],
                       testIndices, copy.copy(times), stats)

    def _featuresStep(self, data: np.array, selectedAttributes: List[str], extMap: List[FeatureExtractor],
                      labels: np.array = None, fit=False):
        """
        ...
        :type fit: bool
        :return: Extracted features and their names
        :rtype: Tuple[csr_matrix, np.array]
        """
        features = []
        featuresNames = []
        ...
        for i, extractor in enumerate(extMap):
            ...
            features.append(actF)

            # names
            if extractor.getName() == "Pass":
                featuresNames.append(attrName)
            else:
                if extractor.featuresNames() is not None:
                    featuresNames += [f"{attrName} : {n}" for n in extractor.featuresNames()]
                else:
                    featuresNames += [f"{attrName} : UNK"] * actF.shape[1]

        return hstack(features).tocsr(), np.array(featuresNames)
...
class ValidatorStratifiedKFold(Validator):
    ...
    def __init__(self, folds: int = 5):
        ...
        self._folds = PluginAttribute("Folds", PluginAttribute.PluginAttributeType.VALUE,
